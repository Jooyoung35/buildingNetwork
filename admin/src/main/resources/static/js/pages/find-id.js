...
function findId() {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('resultInfo').textContent = '가입일: ' + (data.createdAt || '정보 없음');
        } else {
            alert(data.message);
...
document.getElementById('email').addEventListener('input', function() {
    if (emailVerified) {
        ...
        document.getElementById('findIdBtn').disabled = true;
